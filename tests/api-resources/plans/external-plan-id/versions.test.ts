...
describe('resource versions', () => {
  test('create: only required params', async () => {
    const responsePromise = client.plans.externalPlanId.versions.create('external_plan_id', { version: 0 });
  ...
  test('create: required and optional params', async () => {
    const response = await client.plans.externalPlanId.versions.create('external_plan_id', {
      version: 0,
      add_adjustments: [
        {
          adjustment: {
  ...
  test('retrieve', async () => {
    const responsePromise = client.plans.externalPlanId.versions.retrieve('external_plan_id', 'version');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.plans.externalPlanId.versions.retrieve('external_plan_id', 'version', {
